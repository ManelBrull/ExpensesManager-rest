...
public class UserRestControllerTest {
    ...
    public void createUser_created() throws Exception {
        ...
        when(core.createUser(any(UserDTO.class))).thenReturn(1L);
        mockMvc.perform(
post("/user").contentType(MediaType.APPLICATION_JSON_UTF8).content(testUtils.toJson(user)))
                .andExpect(status().is(HttpStatus.CREATED.value()))
                .andExpect(MockMvcResultMatchers.redirectedUrl("http://localhost/user/1"));

        verify(core, times(1)).createUser(argThat(MatcherDTO.getUserMatcher(user)));
    ...
    public void createUser_nullemail() throws Exception {
        ...
        UserDTO user = testUtils.generateValidUserDTO();
        user.setEmail(null);
        mockMvc.perform(post("/user").contentType(MediaType.APPLICATION_JSON_UTF8).content(testUtils.toJson(user)))
